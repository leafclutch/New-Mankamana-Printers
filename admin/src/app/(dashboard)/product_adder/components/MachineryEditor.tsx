...
import { updateMachineryProduct } from "../machinery-service";
import { MachineryOptionsSection } from "./MachineryOptionsSection";
import { MachineryPricingSection } from "./MachineryPricingSection";
import type { MachineryGroup, MachineryProduct, MachineryOption } from "../machinery-types";

interface Props {
  product: MachineryProduct;
  groups: MachineryGroup[];
  onUpdated: (p: MachineryProduct) => void;
}

export function MachineryEditor({ product, groups, onUpdated }: Props) {
  ...
  const [options, setOptions] = useState<MachineryOption[]>(product.options);

  async function handleSave() {
    if (!name.trim()) return toast({ title: "Name cannot be empty", variant: "destructive" });
    setSaving(true);
    try {
      await updateMachineryProduct(product.id, { name: name.trim(), description: desc.trim() || undefined });
  ...
  function handleOptionsChange(opts: MachineryOption[]) {
    setOptions(opts);
  ...
  return (
    <div className="h-full flex flex-col overflow-y-auto">
        ...
        {editing ? (
          ...
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="text-right shrink-0">
              ...
              {groupName && <div className="text-xs text-slate-400 mt-0.5">{groupName}</div>}
            </div>
          </div>
        )}
      </div>
      ...
      <div className="flex-1 px-6 py-6 space-y-8">
        <MachineryOptionsSection product={{ ...product, options }} onOptionsChange={handleOptionsChange} />
        <div className="border-t border-slate-100 dark:border-slate-800" />
        <MachineryPricingSection productId={product.id} options={options} />
      </div>
    </div>
  );
}
