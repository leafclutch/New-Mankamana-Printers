...
import {
  createMachineryField, updateMachineryField, deleteMachineryField,
  createMachineryChoice, updateMachineryChoice, deleteMachineryChoice,
} from "../machinery-service";
import type { MachineryProduct, MachineryOption } from "../machinery-types";

interface Props {
  product: MachineryProduct;
  onOptionsChange: (opts: MachineryOption[]) => void;
}

export function MachineryOptionsSection({ product, onOptionsChange }: Props) {
  ...
  async function handleAddOption() {
    ...
    setAddingOpt(true);
    try {
      const opt = await createMachineryField(product.id, newOptLabel.trim(), affectsPrice);
  ...
  function startEditOpt(opt: MachineryOption) {
  ...
  async function handleSaveOpt(optId: string) {
    ...
    try {
      await updateMachineryField(optId, editOptLabel.trim(), editOptPricing);
  ...
  async function handleAddChoice(optId: string) {
    ...
    try {
      const c = await createMachineryChoice(optId, label);
      onOptionsChange(product.options.map(o =>
  ...
  async function handleSaveChoice(optId: string, choiceId: string) {
    ...
    try {
      await updateMachineryChoice(choiceId, editChoiceLabel.trim());
      onOptionsChange(product.options.map(o =>
        o.id === optId
  ...
  async function handleDeleteChoice(optId: string, choiceId: string) {
    ...
    try {
      await deleteMachineryChoice(choiceId);
      onOptionsChange(product.options.map(o =>
    ...
    <section className="space-y-4">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-0.5">
          What can customers choose when ordering? e.g. Model, Configuration, Capacity.
        </p>
      </div>
            ...
            {editingOptId === opt.id ? (
              ...
              <div className="flex items-center gap-2 min-w-0 flex-1">
                ...
                <button type="button" onClick={() => startEditOpt(opt)}
                  className="shrink-0 p-1 rounded text-slate-300 hover:text-slate-600 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors">
              ...
              <span key={c.id} className="group/chip inline-flex items-center gap-1 px-3 py-1 rounded-full bg-slate-100 dark:bg-slate-700 text-sm text-slate-700 dark:text-slate-200">
                {editingChoiceId === c.id ? (
                  <span className="flex items-center gap-1">
                    <input value={editChoiceLabel} onChange={e => setEditChoiceLabel(e.target.value)}
                      onKeyDown={e => { if (e.key === "Enter") handleSaveChoice(opt.id, c.id); if (e.key === "Escape") setEditingChoiceId(null); }}
                      className="w-24 h-5 text-xs px-1 rounded border border-slate-300 dark:border-slate-500 bg-white dark:bg-slate-800 focus:outline-none" autoFocus />
                    ...
                    <button type="button" onClick={() => setEditingChoiceId(null)} className="text-slate-400 hover:text-slate-600 transition-colors">
                      <X className="h-3 w-3" />
                    </button>
                  </span>
                ) : (
                  <>
                    ...
                    <button type="button" disabled={deletingChoiceId === c.id} onClick={() => handleDeleteChoice(opt.id, c.id)}
      ...
      <div className="rounded-xl border border-dashed border-slate-300 dark:border-slate-600 p-4 space-y-2">
        ...
        <div className="flex gap-2">
          <Input placeholder='e.g. "Model" or "Capacity"' value={newOptLabel}
